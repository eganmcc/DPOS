...
- Void allowed only for `COMPLETED` orders; it appends an immutable `OrderVoid` (at most one per order via `UNIQUE(order_id)`) and never mutates the order. **Superseded (specs/005, specs/006, 2026-09-25):** an OWNER or MANAGER self-authorises, a CASHIER needs an approver PIN, a UMI merchant needs neither; the window is the same business day (Asia/Jakarta); and a sale that has **already been partly refunded cannot be voided** (`VOID_AFTER_REFUND`) — the refund has reversed part of the money and stock already. Refund, full and per-item, ships (specs/005), and an online (platform-paid) sale is refused in-app (`REFUND_NOT_IN_APP`).
