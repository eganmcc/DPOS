...
/** Password the owner fixture answers to — the portal's email/password door. */
export const OWNER_PASSWORD = 'owner-test-123';
...
export async function makeMerchant(
  ...
  // Email + password too: the owner is also the portal-login subject (loginOwner).
  const ownerEmail = `owner-${uuidv4()}@test.invalid`;
  const owner = await prisma.staff.create({
    data: {
      ...
      pinHash: 'x',
      email: ownerEmail,
      passwordHash: await bcrypt.hash(OWNER_PASSWORD, 8),
    },
  });
