    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            if (_photo == null) _intro(t) else _photoPreview(),
      ...
      child: Column(
        children: [
          Container(
            ...
            child: Icon(Icons.receipt_long_outlined, size: 34, color: cs.primary),
          ),
    ...
    return GestureDetector(
      onTap: () => showDialog<void>(
        ...
        builder: (_) => Dialog(
          ...
          child: InteractiveViewer(maxScale: 5, child: Image.file(_photo!)),
        ),
      ),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(16),
        child: Stack(
          children: [
            Image.file(_photo!, height: 240, width: double.infinity, fit: BoxFit.cover),
            Positioned(
              right: 8,
              bottom: 8,
              child: Container(
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 16),
          Text(t.notaReading, style: const TextStyle(fontWeight: FontWeight.w700)),
          const SizedBox(height: 4),
          Text(t.notaReadingHint, style: TextStyle(fontSize: 12, color: cs.onSurfaceVariant)),
        ],
      ),
    );
  }
          ...
          children: [
            SizedBox(
              ...
              child: FilledButton.icon(
                ...
                icon: Icon(done ? Icons.add_a_photo_outlined : Icons.photo_camera_outlined),
                label: Text(
...
class _ResultView extends StatelessWidget {
      ...
      children: [
        ...
        _card(context, [
          ...
          _field(
            ...
            r.notaDate == null ? null : DateFormat('dd-MM-yyyy').format(r.notaDate!),
            unreadable,
          ),
        ...
        _card(context, [
          Text(t.notaItems, style: const TextStyle(fontWeight: FontWeight.w700)),
          const SizedBox(height: 8),
          if (r.items.isEmpty)
          ...
          if (mismatch)
            Padding(
              ...
              child: _banner(context, Icons.warning_amber_rounded, t.notaTotalMismatch),
            ),
        ]),
          ...
          _card(context, [
            Row(
              children: [
                ...
                Text(t.notaUnclearTitle, style: const TextStyle(fontWeight: FontWeight.w700)),
              ],
            ),
            const SizedBox(height: 6),
            for (final u in r.unclear)
              Padding(
                ...
                child: Text('•  $u', style: TextStyle(color: cs.onSurfaceVariant)),
              ),
          ]),
        ],
        const SizedBox(height: 12),
  ...
  Widget _itemsTable(BuildContext context, AppLocalizations t) {
    final cs = Theme.of(context).colorScheme;
    final head = TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: cs.onSurfaceVariant);
    String money(int? v) => v == null ? '—' : formatRupiah(v);
    String qty(num? v) => v == null ? '—' : (v == v.roundToDouble() ? '${v.toInt()}' : '$v');
      ...
      children: [
        ...
        for (final l in reading.items)
          TableRow(children: [
            ...
            Text(money(l.unitPrice), textAlign: TextAlign.right, style: const TextStyle(fontSize: 12)),
  ...
  /// is obvious.
  Widget _field(BuildContext context, String label, String? value, String unreadable) {
    final cs = Theme.of(context).colorScheme;
    return Padding(
